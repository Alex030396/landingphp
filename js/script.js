// Todo el código JavaScript de tu página (el que estaba entre <script> tags)
// Lo he movido a un archivo separado para mejor organización
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function guardarDatos(formData) {
        return new Promise((resolve, reject) => {
            ...
            formData.nonce = document.getElementById('oftalmi_nonce').value;
            
            // Realizar petición AJAX
            jQuery.ajax({
                url: oftalmi_ajax.ajax_url,
                type: 'POST',
                data: {
                    action: 'procesar_formulario_oftalmi',
                    ...formData
                },
                success: function(response) {
                    if (response.success) {
                        resolve({ exito: true, mensaje: response.data });
                    } else {
                        resolve({ exito: false, mensaje: response.data });
                    }
                },
                error: function() {
                    resolve({ exito: false, mensaje: 'Error de conexión. Por favor, intente nuevamente.' });
                }
            });
        });
    };
    ...
    form.addEventListener('submit', async function(e) {
       ...
       if (isNombreValid && isCedulaValid && isMppsValid && isTelefonoValid && isCorreoValid && 
        ...
        const formData = {
            ...
            nivel_residencia: nivelResidencia,
            direccion: lugarVive
        };
